/**
 * 뇌기능검사 보고서 파일(PDF) 업로드용 서명 URL 발급 API (관리자 전용)
 * [보안] middleware.ts는 /api/admin/*을 보호하지 않으므로 이 세션 확인이 유일한 인증 게이트입니다.
 * 실제 파일 바이트는 이 서버를 거치지 않습니다 — 클라이언트가 여기서 받은 서명 URL로
 * Supabase Storage에 직접 업로드합니다.
 */
import { NextResponse } from "next/server";
import { isAdminLoggedIn } from "@/lib/auth";
import { createBrainFileUploadTarget, getChild } from "@/lib/data";
...
export async function POST(req: Request) {
  ...
  try {
    const target = await createBrainFileUploadTarget(body.childId, body.filename, body.contentType);
